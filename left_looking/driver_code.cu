...
int main()
{
    ...
    {
        left_looking_kernel<<<grid, block, 1 * TILE_SIZE * TILE_SIZE * sizeof(float)>>>(d_M, INPUT_SIZE);
    }
    else if((no_of_tiles + 2) * TILE_SIZE * TILE_SIZE * sizeof(float) < devp.sharedMemPerBlock)
    {
        left_looking_kernel<<<grid, block, (no_of_tiles + 2) * TILE_SIZE * TILE_SIZE * sizeof(float)>>>(d_M, INPUT_SIZE);
    }
    else
    {
        left_looking_kernel_less_mem<<<grid, block, 4 * TILE_SIZE * TILE_SIZE * sizeof(float)>>>(d_M, INPUT_SIZE);
    }
